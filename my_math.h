...
#ifndef TCODE_MY_MATH_H
#define TCODE_MY_MATH_H
...
inline void MultiplyVector3andFloat(Vector3f dst, const Vector3f u, float k) {
    dst[0] = u[0] * k;
    dst[1] = u[1] * k;
    dst[2] = u[2] * k;
}

inline void DivVector3andFloat(Vector3f dst, const Vector3f u, float k) {
    dst[0] = u[0] / k;
    dst[1] = u[1] / k;
    dst[2] = u[2] / k;
}

inline void MultiplyVector3ByElement(Vector3f dst, const Vector3f u, const Vector3f v) {
    for (int i = 0; i < 3; i++) {
        dst[i] = u[i] * v[i];
    }
}

inline void DivVector3ByElement(Vector3f dst, const Vector3f u, const Vector3f v) {
    for (int i = 0; i < 3; i++) {
        dst[i] = u[i] / v[i];
    }
}
...
inline void CopyVector2(Vector2f dst, const Vector2f src) {
    memcpy(dst, src, sizeof(Vector2f));
}

inline void SubVector3(Vector3f result, const Vector3f u, const Vector3f v) {
    for (int i = 0; i < 3; i++) {
        result[i] = u[i] - v[i];
    }
}

inline void AddVector3(Vector3f result, const Vector3f u, const Vector3f v) {
    for (int i = 0; i < 3; i++) {
        result[i] = u[i] + v[i];
    }
}

inline float DotVector3(const Vector3f u, const Vector3f v) {
    return u[0] * v[0] + u[1] * v[1] + u[2] * v[2];
}

typedef float Vector4f[4];

inline void LoadVector4(Vector3f v, const float i, const float j, const float k, const float z) {
    v[0] = i;
    v[1] = j;
    v[2] = k;
    v[3] = z;
...
inline void LoadVector2(Vector2f v, const float i, const float j) {
    v[0] = i;
    v[1] = j;
}
...
inline void PersProjectionMatrix44(Matrix44f m, PersProjInfo p) {
    ...
    m[2] = 0.0f;
    m[6] = 0.0f;
    m[10] = (-p.zNear - p.zFar) / zRange;
    m[14] = 2.0f * p.zFar * p.zNear / zRange;
    m[3] = 0.0f;
    m[7] = 0.0f;
    m[11] = 1.0f;
    m[15] = 0.0;
